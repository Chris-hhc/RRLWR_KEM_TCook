...
static int test_kem_negative() {
  ...
  unsigned char cz[RRLWR_KEM_CT_LEN + RRLWR_KEM_SEED_Z_LEN];
  ...
  for(unsigned int i = 0; i < RRLWR_KEM_CT_LEN; i++) {
    ...
    for(unsigned int j = 0; j < RRLWR_KEM_CT_LEN; j++) {
      cz[j] = ct[j];
    }
    for(unsigned int j = 0; j < RRLWR_KEM_SEED_Z_LEN; j++) {
      cz[RRLWR_KEM_CT_LEN+j] = sk[RRLWR_PKE_SK_LEN + RRLWR_PKE_PK_LEN + RRLWR_KEM_HPK_LEN + j];
    }
    RRLWR_KEM_HASH_H(ss3, RRLWR_KEM_SS_LEN, cz, RRLWR_KEM_CT_LEN + RRLWR_KEM_SEED_Z_LEN);
